import { json_stringify } from './json_wrapper.js'
...
let state = {
	...
	message: '',
	impacts: [],
	players: [{ score: 0, pseudo: 'player0', angle: 0, minAngle: 0, maxAngle: 0, paddleSize: 10 }],
	changeColor: false
}
...
function drawPredictionsRotated(ctx: CanvasRenderingContext2D, ball: Point, impacts: Impact[], center: Point, angle: number) {
...
function initBabylonVisuals() {
	...
	state.players.forEach((p: any, i: number) => {
		...
		// BACKGROUND — 2 zones
		const bg1 = createArcMesh(
			`bg1_${i}`,
			centerX,
			centerY,
			radius + paddleWidth / 2,
			p.minAngle + anglePlayer,
			aStart,
			paddleWidth,
			scene
		)
		const bg2 = createArcMesh(
			`bg2_${i}`,
			centerX,
			centerY,
			radius + paddleWidth / 2,
			aEnd,
			p.maxAngle + anglePlayer,
			paddleWidth,
			scene
		)
		...
		// PADDLE glossy
		const pad = createArcMesh(`pad_${i}`, centerX, centerY, radius + paddleWidth / 2, aStart, aEnd, paddleWidth, scene)
		pad.material = matPad
