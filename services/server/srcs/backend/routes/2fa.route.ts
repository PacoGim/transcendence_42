...
export async function create2FAChallenge(userData: any) {
	...
	return { status: res.status, message: res.message, code }
}
...
export async function send2FACode(req: FastifyRequest, reply: FastifyReply) {
	...
	try {
		if (res.code) await sendEmail(email, res.code)
	} catch (error) {
...
export async function validate2FACode(req: FastifyRequest, reply: FastifyReply): Promise<void> {
	...
	if (!inputCode || !purpose || userId == null || has_2fa == null || !email || !username)
		return reply.status(400).send({ message: 'Bad Request. Missing required fields.' })
	...
	let userInfo: InfoFetchType = {
		email: email,
		username: username,
		id: userId,
		has_2fa: has_2fa,
		info: { status: 200, message: 'User info for token' }
	}
